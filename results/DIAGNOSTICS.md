...
## G. recon —— 已撤回(fall-back 假象)
原 §G(recon 抬分至 0.524、~1/8 token 效率)基于**错误结果**:pre-fix 的 buggy `latent_end` 被训去预测第一个
local,推理时模型把 `$LOCALS` 当文本重吐,实为退化的 explicit-trace 生成。latent-mode 复评(latent 块后强制
`<|action_sep|>`)后 recon 跌回 vanilla-CODI 区间(1.5b rw1 ck1000:0.524→**0.444**),非 recon 对照不变
(frozen 3b logit 0.495→0.495)。正确的 recon 结论待 off-path `_fix` 跑完再给。
